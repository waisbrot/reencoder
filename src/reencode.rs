...
impl crate::module::Module for Reencode {
    ...
    fn module_iteration(&self, connection: &Connection) -> Result<()> {
        ...
        while !done {
            ...
            let rows = connection.query("SELECT path FROM paths INNER JOIN video_extensions USING(extension) WHERE extension != $1 or codec != $2 ORDER BY path DESC OFFSET $3::int4 LIMIT $4::int4", &[&target_extension, &target_codec, &offset, &limit])?;
            debug!("Got {:?}", rows);
            offset += limit;
